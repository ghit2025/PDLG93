...
public class AnSemantico {
	...
	public String buscaTipoTS(String lexema) {
		String ambitoActual = lexico.dentroDeFuncion() ? lexico.getAmbitoActual() : "global";
		Map atributosLocal = null;
		Map atributosGlobal = null;

		// Busca en el ámbito local (si estamos en una función)
		if (!"global".equals(ambitoActual)) {
			for (Map.Entry<String, Map<String, Object>> entrada : tablaSimbolos.entrySet()) {
				...
				if (entrada.getKey().equals(lexema) && ambito != null && ambito.equals(ambitoActual)) {
					atributosLocal = atributos;
					break;
				}
			}
		}

		// Si no se ha encontrado en local, busca en global
		if (atributosLocal == null) {
			for (Map.Entry<String, Map<String, Object>> entrada : tablaSimbolos.entrySet()) {
				Map atributos = entrada.getValue();
				String ambito = (String) atributos.get("ambito");
				if (entrada.getKey().equals(lexema) && (ambito == null || "global".equals(ambito))) {
					atributosGlobal = atributos;
					break;
				}
			}
		}

		Map atributos = atributosLocal != null ? atributosLocal : atributosGlobal;
	...
	public void validarDeclaracion(String lexema, String tipo) {
	    ...
	    for (Map.Entry<String, Map<String, Object>> entrada : tablaSimbolos.entrySet()) {
	        ...
	        Map attr = entrada.getValue();
	        String ambito = (String) attr.get("ambito");

	        if (nombre.equals(lexema)) {
	            // ❌ Error si estás declarando una local y ya existe como global
	            if (!"global".equals(ambitoActual) && "global".equals(ambito)) {
	                throw new RuntimeException("Error semántico: Variable '" + lexema +
	                    "' ya declarada como global. No se puede redeclarar como local en la línea " +
	                    lexico.getLinea() + ".");
	            }

	            // ❌ Error si ya existe una variable en el mismo ámbito
	            if (ambitoActual.equals(ambito)) {
	                throw new RuntimeException("Error semántico: Variable '" + lexema +
	                    "' ya declarada en el ámbito actual (línea " + lexico.getLinea() + ")");
	            }
	        }
	    }

	    // Si no hay conflicto, crear o actualizar la entrada
	    Map<String, Object> atributos = tablaSimbolos.getOrDefault(lexema, new HashMap<>());
	    atributos.put("tipo", tipo);
	    atributos.put("categoria", "variable");
	    atributos.put("ambito", ambitoActual);
	    atributos.put("despl", tablaSimbolos.containsKey(lexema)
	        ? tablaSimbolos.get(lexema).get("despl")
	        : null);
	    tablaSimbolos.put(lexema, atributos);
	}
	...
	public void validarDeclaracionFuncion(String lexema, String tipoRetorno, int numParam,
		...
		for (int i = 0; i < parametros.size(); i++) {
		    Map<String, Object> param = parametros.get(i);
		    String tipoParam = (String) param.get("tipo");
		    atributos.put("TipoParam0" + (i + 1), tipoParam);
		}
	...
	public String getTipoRetornoFuncion(String lexema) {
	    if (!tablaSimbolos.containsKey(lexema)
	        || !"funcion".equals(tablaSimbolos.get(lexema).get("tipo"))) {
	        throw new RuntimeException("Error semantico: '" + lexema +
	                "' no es una funcion en la linea " + lexico.getLinea());
	    }
	    return (String) tablaSimbolos.get(lexema).get("TipoRetorno");
	}
